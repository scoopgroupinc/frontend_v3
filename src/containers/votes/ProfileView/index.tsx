...
import LikeButtonsView from "src/components/molecule/LikeButtonsView";
import {
  RemoveActiveMatch,
  selectUserMatches,
...
import { USER_SWIPER_ACTION } from "src/graphql/profile/mutations";
...
export const ProfileView = () => {
    ...
    handleLikeDislike({
      ...
      onCompleted: (data) => {
        ...
        if (message === "match created") {
          ...
          navigation.navigate(VOTES.MATCH_SCREEN_VOTE);
        } else {
          dispatch(
            RemoveActiveMatch({
  ...
  const getDrinkDetails = () => {
    ...
    const drinkName = drink?.userTags[0]?.tagName;
    if (drink?.visible && drinkName) {
      return (
        <Text style={[styles.descriptionText]}>
          {drink?.emoji} {drinkName}
        </Text>
      );
    }
  };
        ...
        >
          ...
          >
            <View style={styles.descriptionContainer}>
              ...
              <View style={styles.section}>
                <Text style={styles.name}>{userMatches[0].choiceName}</Text>
